...
import { getAllAnime, upsertAnime, deleteAnime, uploadArchiveAsset } from "../actions";
import { Anime } from "@/data/mockAnime";
import { motion, AnimatePresence } from "framer-motion";
import { Settings, Plus, Edit, Trash2, X, Save, AlertTriangle, ShieldCheck, ImageIcon, CheckCircle2, Circle, Upload, Loader2 } from "lucide-react";
...
export default function UserVaultDashboard() {
  ...
  const handleAddNew = () => {
    // For users, maybe we don't have a separate /user/add page
    // Let's just open the modal directly for adding
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>, type: 'poster' | 'preview', index?: number) => {
  ...
  const handleSave = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (animeResult.success) {
        setMessage({ type: 'success', text: `Anime ${animeResult.action === 'created' ? 'Forged' : 'Modified'} Successfully` });
        setIsFormOpen(false);
        fetchAnime();
    } else {
          ...
          <div className="relative">
            ...
            <div className="flex items-center gap-6 mb-4">
              ...
              <h1 className="text-6xl sm:text-7xl font-black uppercase tracking-tighter italic leading-none">
                User <br /> 
                <span className="bg-manga-ink text-manga-paper px-4 transition-colors">Vault</span>
              </h1>
            </div>
            <p className="text-xl font-bold border-l-[6px] border-manga-ink pl-6 transition-colors font-geist-sans uppercase">
              STANDARD ACCESS GRANTED: {session.user?.name?.toUpperCase()}
            </p>
          </div>
